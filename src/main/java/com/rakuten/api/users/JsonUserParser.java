...
public class JsonUserParser {
  ...
  public RakutenUser parse(String url) throws MalformedURLException, IOException{
//    String url=this.url+username+".json";
    ...
    while (!parser.isClosed()) {
      ...
      if(JsonToken.FIELD_NAME.equals(token)){
        ...
        }else if (JsonToken.FIELD_NAME.equals(token) && this.employeeNoLabel.equals(parser.getCurrentName())) {
          token = parser.nextToken();
          ru.setEmployeeNo(parser.getText());
        }else if (JsonToken.FIELD_NAME.equals(token) && this.emailLabel.equals(parser.getCurrentName())) {
          token = parser.nextToken();
          ru.setEmail(parser.getText());
        }else if (this.organizationsLabel.equals(parser.getCurrentName())) {
  ...
  public boolean isUsernameExists(String url)throws JsonParseException, IOException{
    return getParser(url)!=null;
  }
  
  public JsonParser getParser(String url)throws JsonParseException, IOException{
    try{
  ...
  public Account getAccount(String url)throws MalformedURLException, IOException{
    return Converter.RakutenUserToAccount(parse(url));
  }
  
  public Account getAccountByUsername(String username)throws MalformedURLException, IOException{
    return Converter.RakutenUserToAccount(parse(this.url+username+".json"));
  }
