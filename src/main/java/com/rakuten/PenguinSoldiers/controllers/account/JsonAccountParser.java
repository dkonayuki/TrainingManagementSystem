...
public class JsonAccountParser {
	...
	public LinkedList<Account> parse() throws MalformedURLException,
			IOException {
		return parse(this.url);
	}

	public LinkedList<Account> parse(String url) throws MalformedURLException,
			IOException {
	...
	public Account parseAccount(JsonParser parser, JsonToken token)
		...
		while (!parser.getText().equals("}")) {
			 ...
			 * this.fullAccountLabel.equals(parser.getCurrentName())) { token =
			 * parser.nextToken(); u.setUsername(parser.getText()); } if
			 * (JsonToken.FIELD_NAME.equals(token) &&
			 * this.nameLabel.equals(parser.getCurrentName())) { token =
			 * parser.nextToken(); u.setName(parser.getText()); }
			 * 
			 * if (JsonToken.FIELD_NAME.equals(token) &&
			 * this.employeeLabel.equals(parser.getCurrentName())) { token =
			 * parser.nextToken(); u.setId(Long.parseLong(parser.getText())); }
			 */
			if (JsonToken.FIELD_NAME.equals(token)
